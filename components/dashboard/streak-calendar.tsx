...
export function StreakCalendar({ currentStreak }: StreakCalendarProps) {
  ...
  useEffect(() => {
    ...
    const fetchMonthActivity = async () => {
      try {
        // Send user's timezone offset to server for correct date calculation
        const offset = new Date().getTimezoneOffset();
        const response = await fetch(`/api/dashboard/streak?offset=${offset}`);
        if (response.ok) {
  ...
  const getActivityLevel = (date: Date): number => {
    // Use local date string to match how the calendar renders dates
  ...
  const getLocalDateString = (date: Date): string => {
    // Get the local date string (YYYY-MM-DD) regardless of timezone
      ...
      <div className="mt-4">
          ...
          {days.map((day, idx) => {
            ...
            return (
              <div
                ...
                style={{
                  backgroundColor: activityLevel > 0 ? "var(--primary)" : isCurrentMonth ? "var(--surface-secondary)" : "transparent",
                  color: activityLevel > 0 ? "#1a1a1a" : isCurrentMonth ? "var(--text-primary)" : "var(--text-muted)",
        ...
        <div className="flex items-center gap-4 mt-4 text-xs" style={{ color: "var(--text-muted)" }}>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded" style={{ backgroundColor: "var(--surface-secondary)" }} />
