  ...
  if (decks.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-500 text-sm">No decks found. Create your first deck to get started!</p>
      </div>
    )
  }
      ...
      {decks.map((deck) => (
        <div
          ...
          className={`rounded-lg border-2 p-6 hover:shadow-lg transition-shadow ${
            deck.color && deck.color in colorClasses
              ? colorClasses[deck.color as keyof typeof colorClasses]
        ...
        >
          ...
          <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-300">
            <div className="text-sm text-gray-600">
              <span className="font-medium">{deck._count.flashcards}</span> cards
            </div>
